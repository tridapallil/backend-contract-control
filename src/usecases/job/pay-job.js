...
const payJob = async req => {
  ...
  if (!isClientProfile(profile)) throw new ValidationError('Not a contractor profile!', 401)
  const job = await getJobById(jobId, profile.id)
  if (!job) throw new ValidationError('Job not found!', 400)
  if (hasBalance(profile, job)) throw new ValidationError('Not enough balance!', 400)
